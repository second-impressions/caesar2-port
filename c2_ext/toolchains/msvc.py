...
from c2_ext.toolchains.watcom import (
...
class MSVCToolchain(Toolchain):
    ...
    # Cflags applied to every scratch compile.  The scratch is bundled
    # (every decl inlined) so no ``/I include`` / ``/FIc2_funcs.h`` is
    # needed; the Watcom-specific calling-convention keywords are
    # stripped via ``-D`` so they parse as nothing.
