    ...
    const BUILD_VERSION = "pr45-dbf10fe0";
    ...
    let importFailureHandled = false;
    let operationTimer;
    ...
    function beginOperation(title, total = 0, detail = "Starting…") {
      ...
      if (total > 0) {
        ...
        operationTimer = setInterval(() => {
          ...
          if (seconds >= 120 && preparingAssets) {
            finishImportFailure("Reading the source catalog timed out after 2 minutes");
          } else {
            operationDetail.textContent = `${detail} (${seconds}s)`;
          }
        }, 1000);
      }
    ...
    function showOperationError(message) {
      ...
      if (!operationDialog.open) operationDialog.showModal();
      if (smokeOutput && query.get("smoke-test") === "reject" &&
          !smokeOutput.some(line => line.includes("game data import rejected visibly"))) {
        const result = `game data import rejected visibly: ${message}`;
        smokeOutput.push(result); console.log(result);
      }
    }
    ...
    function finishImportFailure(message) {
      if (importFailureHandled) return;
      importFailureHandled = true;
      gameRunning = false;
    ...
    var Module = {
      ...
      onImportError(message) {
        pendingImportError = message || "game-data import failed without an explanation";
        /* OPFS keeps the Emscripten runtime alive after the prepare command
         * exits, so Module.onExit is not a reliable failure notification.
         * The C main thread calls this bridge after joining the import worker;
         * finish the visible operation here instead of waiting for an exit
         * callback that may never come. */
        if (preparingAssets) finishImportFailure(pendingImportError);
      },
      onImportProgress(phase, completedKiB, totalKiB,
                       completedFiles, totalFiles) {
        if (!preparingAssets) return;
      ...
      onAbort(reason) {
        ...
        if (preparingAssets || importFailureHandled) {
          finishImportFailure(message);
          return;
        }
        gameRunning = false; panel.hidden = false;
        document.body.classList.remove("playing");
        const previous = localStorage.getItem(ACTIVE_SOURCE);
        endOperation();
