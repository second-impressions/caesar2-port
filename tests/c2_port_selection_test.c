...
{
    c2_port_region_selection_begin(500, 240);
    TEST_ASSERT_TRUE(c2_port_region_selection_consume_release(500, 240));
    TEST_ASSERT_FALSE(c2_port_region_selection_consume_release(500, 240));
}

static void test_pointer_jitter_is_still_a_click(void)
{
    c2_port_region_selection_begin(500, 240);
    TEST_ASSERT_TRUE(c2_port_region_selection_consume_release(504, 236));
}

static void test_drag_release_is_not_consumed(void)
{
    c2_port_region_selection_begin(500, 240);
    TEST_ASSERT_FALSE(c2_port_region_selection_consume_release(480, 240));
    TEST_ASSERT_FALSE(c2_port_region_selection_consume_release(500, 240));
}
