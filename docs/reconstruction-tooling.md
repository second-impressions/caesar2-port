...
The byte-exact DOS reconstruction this port continues from, and the tooling
that still lives in this repository for comparing against the original
`PS.EXE`. Nothing here is needed to build or play the port.
...
## Running the rebuilt DOS executable
