 ...
 * neighbouring variable, and that cannot be respelled byte-exactly in the
 * reconstruction (the ones that can are fixed there: the folded table
 * references in cap_land_value, get_query_info, one_aquaduct_ramification
 * and evolve_water_supply_baths_industry). Verified against the PS.EXE data
 * segment:
 *
 ...
 *   named shorts, so the file format is unchanged (two table entries more).
